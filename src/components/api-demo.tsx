...
export function ApiDemo() {
  ...
  const groupedFixtures = useMemo(
    () => (data ? groupFixturesByDay(data.fixtures) : []),
    [data]
  );
  ...
  useEffect(() => {
    ...
    if (!activeDay || !groupedFixtures.some((group) => group.dayKey === activeDay)) {
      setActiveDay(groupedFixtures[0].dayKey);
    }
  }, [activeDay, groupedFixtures]);

  const selectedGroup = groupedFixtures.find((group) => group.dayKey === activeDay);
    ...
    <section className="grid gap-6 rounded-[2rem] border border-white/60 bg-white/80 p-5 shadow-[0_24px_80px_rgba(15,23,42,0.12)] backdrop-blur sm:p-7">
      <div className="grid gap-4 rounded-[1.75rem] bg-[linear-gradient(135deg,#0b132b_0%,#124e78_48%,#6ee7f9_100%)] p-6 text-white shadow-[0_20px_60px_rgba(18,78,120,0.32)]">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          <div className="rounded-2xl bg-white/12 px-4 py-3 backdrop-blur">
            ...
            <p className="mt-1 text-base font-semibold">{data?.from ?? startDate}</p>
          </div>
          <div className="rounded-2xl bg-white/12 px-4 py-3 backdrop-blur">
            <p className="text-xs uppercase tracking-[0.22em] text-cyan-100/75">
              End
            </p>
            <p className="mt-1 text-base font-semibold">{data?.to ?? "..."}</p>
          </div>
          <div className="rounded-2xl bg-white/12 px-4 py-3 backdrop-blur">
            <p className="text-xs uppercase tracking-[0.22em] text-cyan-100/75">
              Matchdays
            ...
            <p className="mt-1 text-base font-semibold">
              {loading ? "Loading..." : `${groupedFixtures.length} days`}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-3 rounded-[1.5rem] border border-cyan-100 bg-[linear-gradient(180deg,rgba(240,249,255,0.95),rgba(255,255,255,0.92))] p-4">
        ...
        <p className="text-sm leading-6 text-slate-600">
          Izberi dan in prikazi samo tekme za ta datum, namesto celega dolgega seznama.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid gap-3 rounded-[1.6rem] border border-cyan-100 bg-[linear-gradient(180deg,rgba(240,249,255,0.85),rgba(255,255,255,0.92))] p-4 shadow-[0_12px_30px_rgba(14,116,144,0.08)]">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-500">
                  Klikni dan za prikaz tekem.
                </p>
              </div>
              <div className="rounded-full bg-cyan-100 px-3 py-1 text-sm font-semibold text-cyan-900">
                {groupedFixtures.length} dni
              </div>
            </div>

            <div className="flex gap-3 overflow-x-auto pb-2">
              {groupedFixtures.map((group) => {
                const isActive = group.dayKey === activeDay;

                return (
                  <button
                    key={group.dayKey}
                    type="button"
                    onClick={() => setActiveDay(group.dayKey)}
                    className={
                      isActive
                  ...
                  >
                    <span className={isActive ? "text-xs uppercase tracking-[0.22em] text-cyan-100/80" : "text-xs uppercase tracking-[0.22em] text-cyan-600"}>
                      {group.shortLabel}
                    </span>
            ...
            <section className="grid gap-4 rounded-[1.6rem] border border-cyan-100 bg-[linear-gradient(180deg,rgba(240,249,255,0.85),rgba(255,255,255,0.92))] p-4 shadow-[0_12px_30px_rgba(14,116,144,0.08)]">
              <div className="flex items-center gap-3">
                <div className="grid h-16 w-16 place-items-center rounded-2xl bg-[linear-gradient(135deg,#0f172a_0%,#0891b2_100%)] text-white shadow-md">
                  <span className="text-base font-semibold tracking-wide">{selectedGroup.shortLabel}</span>
                </div>
                <div>
                  <h3 className="text-lg font-semibold uppercase tracking-[0.2em] text-cyan-800">
                    {selectedGroup.dayLabel}
                  </h3>
                  <p className="mt-1 text-sm text-slate-500">
                    {selectedGroup.fixtures.length} {selectedGroup.fixtures.length === 1 ? "tekma" : "tekme"} na izbran dan
                  </p>
                </div>
              </div>
        ...
        <div className="rounded-[1.5rem] border border-dashed border-cyan-200 bg-[linear-gradient(180deg,#f0f9ff_0%,#ffffff_100%)] px-5 py-8 text-sm text-slate-600">
          No fixtures were returned for this date range or league access level.
        </div>
      )}
    </section>
  );
